...
describe('number of callback attempts', () => {
    it('should be taken from env variable', () => {
        ...
        expect(contentReplaceSpy.mock.calls[0][0]).toEqual(<CallbackAndVoicemail key="callback-component" maxAttempts={5} />);
    });
    ...
    it('should create error if no. of attempts in invalid and use default value', () => {
        ...
        replaceViewForCallbackAndVoicemail(flex, manager);
        expect(errorManagerSpy.mock.calls[0][0]).toEqual("Invalid number of retry attempts, setting it to default value of 3");
        expect(contentReplaceSpy.mock.calls[0][0]).toEqual(<CallbackAndVoicemail key="callback-component" maxAttempts={3} />);
    });
});